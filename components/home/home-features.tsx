...
const FEATURES = [
  { icon: <BeanIcon />, title: 'Premium Beans', body: 'Carefully selected coffee with a rich, balanced profile.' },
  { icon: <PackageIcon />, title: 'Freshly Packed', body: 'Packed with care to preserve aroma and freshness.' },
  { icon: <HeartHandshakeIcon />, title: 'Made With Care', body: 'A warm coffee experience from order to cup.' },
] as const
      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid grid-cols-1 gap-0 divide-y divide-[#B6885E]/12 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
          {FEATURES.map((feature) => (
            <div key={feature.title} className="flex flex-col items-center gap-4 px-8 py-10 text-center sm:px-10">
              <div className="flex h-16 w-16 items-center justify-center rounded-full border border-[#B6885E]/28 bg-[#1B140F]/80 text-[#D6A373] shadow-[0_0_28px_rgba(182,136,94,0.14)]">
                {feature.icon}
              </div>
              <div>
                <h3 className="font-heading mb-2 text-lg font-semibold text-[#F5E6D8]">{feature.title}</h3>
                <p className="text-sm leading-relaxed text-[#D6B79A]/64">{feature.body}</p>
              </div>
            </div>
          ))}
        </div>
